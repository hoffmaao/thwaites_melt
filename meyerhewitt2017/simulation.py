...
# EbarFun = @(tau)Qbar-np.cos(2*pi*tau);

#% Initial values
Rbar = 0
#% fixed surface water flux (rain)
pressure0 = -(pc(1) / B) * np.ones(N)
#% initial pressure
zs = 0
...
for n in range(Nt):  # range(Nt):
    ...
    FpE, FmE = enthalpydiffusiveflux(Theta_nm1, phi_nm1, S_nm1, dt, dx, U / B, Stefan)
    ...
    Fm = FadvImH + FmS + FmT
    # print(EbarFun(n*dt))
    Fm[0] = Stefan * (EbarFun(n * dt) - Theta_nm1[0]) + Stefan * Rbar
    ...
    if np.mod(n, plot_amount) == 0:
        ...
        fig.savefig(
            "/Users/andrew/projects/thwaites_melt/figures/fig01_t{}.png".format(
                n * dt
            )
        )
...
fig.savefig("/Users/andrew/projects/thwaites_melt/figures/fig02.png")
