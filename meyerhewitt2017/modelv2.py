...
def energy_balance_ice(temperature_ice,temperature_ambient,h_i,h_s,Q_sw,Q_lw,u,P,q):
    r"""
    ...
    return Q_sens + Q_lat + ϵ_i*σ*temperature_ice**4 - (1-α_s)*Q_sw - ϵ_i*Q_lw  
...
def conversiontotemperature(H,W,Stefan):
	...
	phi = np.maximum(1 - W + np.maximum(0.0,H/(ρ*L)),0.0);
	#% Sc = max(0,(H./(Stefan*phi)));
	#% S = max(0,H)./(max(0,H)+Stefan*(1-W));
	S = np.maximum(np.minimum(1,np.maximum(0,H/(ρ*L*phi))),0);
...
def saturationdiffusiveflux(Porosity,Saturation,dt,dx,Constants):
	...
	fim = (2/dx)*(((1./Dm)+(1./D))**(-1));
	fip = (2/dx)*(((1./D)+(1./Dp))**(-1));
	Fm = ρ * g / μ * γ / dp *fim*(Saturation-Saturationm);
	Fp = ρ * g / μ * γ / dp *fip*(Saturationp-Saturation);
